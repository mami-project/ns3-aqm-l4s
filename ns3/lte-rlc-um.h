...
public:
  ...
  // AQM Mechanisms
  virtual double GetQueueDelay (Ptr<LteRlcUm> handle);
  virtual double GetTimeStamp (Ptr<Packet> packet);
  Ptr<UniformRandomVariable> m_uv;
  int m_secToken;
  double m_dropProb;
  bool m_qIsInit;
  bool m_fIsInit;
  Ptr<LteRlcUm> m_queueHandle;
  std::ofstream* m_qDelayFile;
  std::ofstream* m_throughputFile;
  std::ofstream* m_actionsFile;
  
  // RED
  virtual bool DoDropRED (Ptr<Packet> p);
  uint32_t m_minThRED;
  uint32_t m_maxThRED;
  double m_maxProbRED;
  
  // PIE
  virtual bool DropEarlyPIE (Ptr<Packet> p);
  virtual bool DropEarlyRED ();
  virtual void CalculateP ();
  virtual void ThroughputTrace (uint64_t bytesCheckedL4S, uint64_t bytesCheckedClassic);
  uint64_t m_enqueuedBytes;
  EventId m_rtrsEventPIE;
  EventId m_throughputTraceEvent;
  double m_aPIE;
  double m_bPIE;
  double m_qDelayRefPIE;
  double m_qDelayOldPIE;
  double m_burstAllowancePIE;
  double m_maxBurstAllowancePIE;
  double m_k;
  
  double m_tUpdatePIE;
  
  uint64_t m_throughputCount;
  
  bool m_l4s;
  bool m_red;
  // End of AQM Meachanisms
  /**
   * MAC SAP
   */
