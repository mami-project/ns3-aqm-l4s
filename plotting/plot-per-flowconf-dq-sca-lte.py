...
for ecn in [0, 1]:
	for ccC in ['reno', 'cubic']:
		for ccL in ['DCTCP', 'Relentless']:
			...
			ccL=ccL.lower()
			for aqm in ["dq-red-sca", "dq-pie-sca"]:
				aqms.append(aqm)
...
fontsizelabel=10
fontsizelegend=10
title="Dual Queue Uncoupled AQM with Scalable Congestion Control (LTE)\nLow-Latency Flows: "+nLF+"  -  Classic Flows: "+nCF+"\n\n"
basename="plots-lte/dq-sca-per-flowconf-"+nLF+":"+nCF+".png"
...
axarr[1].text(0.3, 1.5, "LL more throughput     ", bbox=dict(facecolor='w', alpha=0.2), fontsize=10, horizontalalignment='left', verticalalignment='center')
