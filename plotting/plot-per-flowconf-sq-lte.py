	...
	for ccC in ['reno', 'cubic']:
		for ccL in [ccC]:
			...
			for aqm in ["sq-red", "sq-pie"]:
				aqms.append(aqm)
				cur.execute("SELECT markRateL, faultRateC, qDelayMaxL, qDelayAvgL, shareqEmptyL, shareqEmptyC, shareqEmptyB, utilization, jainmod, dropRateC, markRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccL+"' AND ecn="+str(ecn)+";")
				rows=cur.fetchall()
...
fontsizelabel=10
fontsizelegend=10
title="Single Queue AQM (LTE)\nLow-Latency Flows: "+nLF+"  -  Classic Flows: "+nCF+"\n\n"
basename="plots-lte/sq-per-flowconf-"+nLF+":"+nCF+".png"
...
# ploting
for i in range(0,2):
	axarr[0].plot(np.arange(0.5, len(labels), 1.0), qDelayMaxL[i::2], linestyle=':', marker='+', markersize=12, color=colors[i], label="Average Queue Delay - "+aqmname[i])
	axarr[0].plot(np.arange(0.5, len(labels), 1.0), qDelayAvgL[i::2], linestyle='-', marker='+', markersize=12, color=colors[i], label="Maximum Queue Delay - "+aqmname[i])
