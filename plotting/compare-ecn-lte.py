	...
	for nLF in [1,5,10]:
		for nCF in [1,5,10]:
			for ccC in ["reno", "cubic"]:
				cur.execute("SELECT qDelayAvgL, qDelayMaxL, qDelayAvgC, qDelayMaxC, shareL, utilization, markRateL, faultRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccC+"' AND ecn=0;")
				rows=cur.fetchall()
				row=rows[0]
				...
				cur.execute("SELECT qDelayAvgL, qDelayMaxL, qDelayAvgC, qDelayMaxC, shareL, utilization, markRateL, faultRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccC+"' AND ecn=1;")
				rows=cur.fetchall()
				row=rows[0]
		...
		for nCF in [1,5,10]:
			for ccC in ["reno", "cubic"]:
				for ccL in ["dctcp", "relentless"]:
					cur.execute("SELECT qDelayAvgL, qDelayMaxL, qDelayAvgC, qDelayMaxC, shareL, utilization, markRateL, faultRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccL+"' AND ecn=0;")
					rows=cur.fetchall()
					row=rows[0]
					scenario0.append(aqm+"-"+str(nLF)+":"+str(nCF)+"-"+ccC+"-"+ccC)
					qDelayAvgL0.append(float(row[0]))
					...
					cur.execute("SELECT qDelayAvgL, qDelayMaxL, qDelayAvgC, qDelayMaxC, shareL, utilization, markRateL, faultRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccL+"' AND ecn=1;")
					rows=cur.fetchall()
					row=rows[0]
					scenario1.append(aqm+"-"+str(nLF)+":"+str(nCF)+"-"+ccC+"-"+ccC)
					qDelayAvgL1.append(float(row[0]))
...
ax2.plot(np.arange(0.5, 108.0, 1.0), shareLr[72:], linestyle='', marker='+', color='b', label="Throughput Share L4S (ECN - No ECN)")
ax2.plot(np.arange(0.5, 108.0, 1.0), utilizationr[72:], linestyle='', marker='+', color='g', label="Link Utilzation (ECN - No ECN)")
ax2.set_xlim([0,108])
ax2.set_ylim([-3,3])
...
f.suptitle("Difference in Throughput Share L4S and Utilization (ECN - No ECN)\nLTE", fontsize=14)
f.set_size_inches(12, 8)
f.savefig("plots-comparison-lte/compare-ecn-throughput.png", dpi=100)
...
ax2.plot(np.arange(0.5, 108.0, 1.0), qDelayMaxLr[72:], linestyle='', marker='+', color='b', label="Maximum Queuing Delay (ECN - No ECN)")
ax2.plot(np.arange(0.5, 108.0, 1.0), qDelayAvgLr[72:], linestyle='', marker='+', color='g', label="Average Queuing Delay (ECN - No ECN)")
ax2.set_xlim([0,108])
...
f.suptitle("Difference in Average and Maximum L4S Queuing Delay (ECN - No ECN)\nLTE", fontsize=14)
f.set_size_inches(12, 8)
f.savefig("plots-comparison-lte/compare-ecn-qDelay.png", dpi=100)
...
ax2.plot(np.arange(0.5, 108.0, 1.0), markRateLr[72:], linestyle='', marker='+', color='b', label="L4S Mark Rate (ECN - No ECN)")
ax2.plot(np.arange(0.5, 108.0, 1.0), faultRateCr[72:], linestyle='', marker='+', color='g', label="Classic Drop/Mark Rate (ECN - No ECN)")
ax2.set_xlim([0,108])
...
f.suptitle("Difference in Drop/Mark Rates (ECN - No ECN)\nLTE", fontsize=14)
f.set_size_inches(12, 8)
f.savefig("plots-comparison-lte/compare-ecn-dropRate.png", dpi=100)
