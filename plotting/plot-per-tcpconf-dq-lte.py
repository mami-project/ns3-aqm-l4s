...
for nLF in [1, 5, 10]:
	for nCF in [1, 5, 10]:
		...
		for aqm in ["dq-red", "dq-pie"]:
			aqms.append(aqm)
			cur.execute("SELECT markRateL, faultRateC, qDelayMaxL, qDelayAvgL, shareqEmptyL, shareqEmptyC, shareqEmptyB, utilization, jainmod, dropRateC, markRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccC+"' AND ecn="+str(ecn)+";")
			rows=cur.fetchall()
...
fontsizelabel=10
fontsizelegend=10
title="Dual Queue Uncoupled AQM with Classic Congestion Control on all Flows (LTE)\nCongestion Control: "+ccCstr+"  -  "+ecnstr+"\n\n"
basename="plots-lte/dq-per-tcpconf-"+ccC+"-"+ccL+"-"+ecnstr2+".png"
