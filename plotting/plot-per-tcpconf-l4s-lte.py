...
fontsizelabel=10
fontsizelegend=10
title="DualQ Coupled AQM (LTE)\nClassic CC: "+ccCstr+"  -  Scalable CC: "+ccLstr+"  -  "+ecnstr+"\n\n"
basename="plots-lte/l4s-per-tcpconf-"+ccC+"-"+ccL+"-"+ecnstr2+".png"
...
axarr[1].text(0.27125, 1.5, "L4S more throughput    ", bbox=dict(facecolor='w', alpha=0.2), fontsize=10, horizontalalignment='left', verticalalignment='center')
...
# ploting
for i in range(0,1):
	axarr[0].plot(np.arange(0.5, len(labels), 1.0), qDelayMaxL[i::1], linestyle=':', marker='+', markersize=12, color=colors[i], label="Maximum Queue Delay - "+aqmname[i])
	axarr[0].plot(np.arange(0.5, len(labels), 1.0), qDelayAvgL[i::1], linestyle='-', marker='+', markersize=12, color=colors[i], label="Average Queue Delay - "+aqmname[i])

for i in range(0,1):
	axarr[1].plot(np.arange(0.5, len(labels), 1.0), utilization[i::1], linestyle=':', marker='+', markersize=12, color=colors[i], label="Link Utilization - "+aqmname[i])
	axarr[1].plot(np.arange(0.5, len(labels), 1.0), jainmod[i::1], linestyle='-', marker='+', markersize=12, color=colors[i], label="Modified Jain Index - "+aqmname[i])
