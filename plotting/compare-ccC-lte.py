		...
		for nLF in [1,5,10]:
			for nCF in [1,5,10]:
				for ccL in ["dctcp", "relentless"]:
					cur.execute("SELECT qDelayAvgL, qDelayMaxL, qDelayAvgC, qDelayMaxC, shareL, utilization, markRateL, faultRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='cubic' AND ccL='"+ccL+"' AND ecn="+ecn+";")
					rows=cur.fetchall()
					row=rows[0]
					qDelayAvgL0.append(float(row[0]))
					qDelayMaxL0.append(float(row[1]))
					...
					cur.execute("SELECT qDelayAvgL, qDelayMaxL, qDelayAvgC, qDelayMaxC, shareL, utilization, markRateL, faultRateC FROM results_lte WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='reno' AND ccL='"+ccL+"' AND ecn="+ecn+";")
					rows=cur.fetchall()
					row=rows[0]
					qDelayAvgL1.append(float(row[0]))
					qDelayMaxL1.append(float(row[1]))
...
ax2.plot(np.arange(0.5, 108.0, 1.0), shareLr[72:], linestyle='', marker='+', color='b', label="Throughput Share L4S (Reno - Cubic)")
ax2.plot(np.arange(0.5, 108.0, 1.0), utilizationr[72:], linestyle='', marker='+', color='g', label="Link Utilization (Reno - Cubic)")
ax2.set_xlim([0,108])
ax2.set_ylim([-3,3])
...
f.suptitle("Difference in Throughput Share L4S and Utilization (Reno - Cubic)\nLTE", fontsize=14)
f.set_size_inches(12, 8)
f.savefig("plots-comparison-lte/compare-ccC-throughput.png", dpi=100)
...
ax2.plot(np.arange(0.5, 108.0, 1.0), qDelayMaxLr[72:], linestyle='', marker='+', color='b', label="Maximum Queuing Delay (Reno - Cubic)")
ax2.plot(np.arange(0.5, 108.0, 1.0), qDelayAvgLr[72:], linestyle='', marker='+', color='g', label="Average Queuing Delay (Reno - Cubic)")
ax2.set_xlim([0,108])
...
f.suptitle("Difference in Average and Maximum L4S Queuing Delay (Reno - Cubic)\nLTE", fontsize=14)
f.set_size_inches(12, 8)
f.savefig("plots-comparison-lte/compare-ccC-qDelay.png", dpi=100)
...
ax2.plot(np.arange(0.5, 108.0, 1.0), markRateLr[72:], linestyle='', marker='+', color='b', label="L4S Mark Rate (Reno - Cubic)")
ax2.plot(np.arange(0.5, 108.0, 1.0), faultRateCr[72:], linestyle='', marker='+', color='g', label="Classic Mark/Drop Rate (Reno - Cubic)")
ax2.set_xlim([0,108])
...
f.suptitle("Difference in Drop Rate (Reno - Cubic)\nLTE", fontsize=14)
f.set_size_inches(12, 8)
f.savefig("plots-comparison-lte/compare-ccC-dropRate.png", dpi=100)
