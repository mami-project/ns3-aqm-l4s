	...
	for ccC in ['reno', 'cubic']:
		for ccL in [ccC]:
			...
			for aqm in ["sq-red", "sq-pie"]:
				aqms.append(aqm)
				cur.execute("SELECT markRateL, faultRateC, qDelayMaxL, qDelayAvgL, shareqEmptyL, shareqEmptyC, shareqEmptyB, utilization, jainmod, dropRateC, markRateC FROM results WHERE aqm='"+aqm+"' AND nLF="+str(nLF)+" AND nCF="+str(nCF)+" AND ccC='"+ccC+"' AND ccL='"+ccL+"' AND ecn="+str(ecn)+";")
				rows=cur.fetchall()
...
basename="plots/sq-per-flowconf-"+nLF+":"+nCF+".png"
